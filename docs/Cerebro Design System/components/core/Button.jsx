...
if (typeof document !== "undefined" && !document.getElementById("cb-btn-css")) { const t = document.createElement("style"); t.id = "cb-btn-css"; t.textContent = css; document.head.appendChild(t); }
export function Button({ variant = "secondary", size = "md", icon, children, fullWidth, disabled, onClick, type = "button", style, className = "" }) {
  const iconSize = size === "sm" ? 14 : 16;
  return (
    <button type={type} disabled={disabled} onClick={onClick}
      className={`cb-btn cb-btn-${size} cb-btn-${variant} ${className}`}
      style={{ width: fullWidth ? "100%" : undefined, ...style }}>
      {icon ? <Icon name={icon} size={iconSize} /> : null}{children}
    </button>
  );
}
