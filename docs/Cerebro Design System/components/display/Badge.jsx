...
if (typeof document !== "undefined" && !document.getElementById("cb-badge-css")) { const t = document.createElement("style"); t.id = "cb-badge-css"; t.textContent = css; document.head.appendChild(t); }
const TONES = {
...
export function Badge({ tone = "neutral", variant = "tint", children, style, className = "" }) {
  const t = TONES[tone] || TONES.neutral;
  return (
    <span className={`cb-badge ${variant === "outline" ? "cb-badge-outline" : ""} ${className}`}
      style={{ background: t.bg, color: t.fg, ...style }}>{children}</span>
  );
}
