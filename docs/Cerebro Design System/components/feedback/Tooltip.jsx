...
if (typeof document !== "undefined" && !document.getElementById("cb-tip-css")) { const t = document.createElement("style"); t.id = "cb-tip-css"; t.textContent = css; document.head.appendChild(t); }
export function Tooltip({ content, kbd, side = "top", children, style, className = "" }) {
  return (
    <span className={`cb-tip ${side === "bottom" ? "cb-tip-bottom" : ""} ${className}`} style={style}>
      {children}
      <span className="cb-tip-bubble" role="tooltip">{content}{kbd ? <span className="cb-tip-kbd">{kbd}</span> : null}</span>
    </span>
  );
}
