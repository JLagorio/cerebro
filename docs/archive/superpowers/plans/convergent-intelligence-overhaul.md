...
section), and multi-dimensional budgets beyond run counts (M26).
...
is the proof nothing was silently dropped. **Matrix Rev 2** (owner freeze
review) pulled ten foundational primitives forward from M28+ into M22–M27 —
schema and contracts, not product surfaces — under the rule *persist the
primitive early when retrofitting would be painful; defer the rich consumer
until it earns itself*. The milestone sections below reflect those pulls; the
matrix records each one.
...
- A projection manifest (path → content hash → generating event id) is the *sole*
...
- Two capture paths, strict priority: **in-app edits emit human_assertion events
  transactionally at the IPC boundary** (the app knows actor/field/before/after —
  no inference); the watcher/hash-diff path covers only out-of-band editors.
...
- guard_human_write's successor is a **capture valve**, not a wall: in-app edits to
  projections transform into assertion proposals (verify_concept is retroactively
  v1 of this); refusal survives only for edits that can't be assertions (forging
  provenance stamps).
...
- Proposals travel **exclusively as serde-validated MCP tool calls** — never stdout
  ...
  accumulate server-side and apply transactionally on a terminal commit call: a run
  that dies mid-stream (quota death) applies nothing.
- A **separate submitProposal channel** returns typed
  `applied {new_version} | rejected {code, rule, expected, actual}` results. The
...
- **Two records, two destinies.** The epistemic ledger receives: valid proposals,
  ...
  log** (app-data runtime DB, see M25) receives: schema mistakes
...
- **Risk ladder** (static Rust table; agent-declared risk can only RAISE):
  LOW auto-apply+journal (append observation, cache_source, draft concept);
  MEDIUM auto+journal+one-click revert (update/supersede of non-human-reviewed
  beliefs, draft→stable, relation edits); HIGH queued human card (anything touching
  a human-reviewed belief, tombstones, deprecations); CRITICAL queued+diff review
  (entity merges, mass-supersede). Tripwire test in the write_target style
  (mcp.rs:1362): no proposal op ships unmapped.
...
- **Maintenance pass** — Reconciler+Temporal+Curiosity+risk-gated Skeptic as ONE
  scheduled run extending the existing jobs.ts lanes (which already are this
  scheduler).
...
- Ambient spend capped **by construction at ~10–20 CLI runs/day**; background LLM
  ...
  Quota exhaustion is modeled as source-health: un-consume the job ledger, backoff
...
content-addressed, immutable object, but its *semantics are distinguished from day
one* via mandatory `observation_kind: source_snapshot | extracted_assertion |
human_assertion | system_event | derived_content`, plus `source_artifact_hash`,
`extractor_version`, `extracted_text`, `raw_pointer` — because "Alice posted
...
PLUS a **human attestation event pinned to the reviewed revision's content hash**.
Attestation feeds world-state confidence ONLY and is structurally excluded from
...
reverts to unverified on any agent revision with no record). sources[] migrate as
**unsnapshotted-reference events** — no git archaeology to fabricate provenance;
only post-migration ingests get true content-addressed Observations.
supersedes/refines/contradicts become belief-relation events. Migrated events carry
`migrated` provenance (event time from original stamps, ingested_at = migration
time) so temporal reasoning never mistakes migration day for a burst of learning.

**Human input has TWO channels, modeled explicitly.**
`human_review_attestation` — "I reviewed Cerebro's synthesis and approve it" —
feeds Validity/world-state only and is structurally excluded from evidence lineage
(prevents the loop: AI writes belief → human clicks Verify → verification becomes
evidence for the belief). `human_firsthand_assertion` — "I'm the hardware lead; I
...
Three ordinal axes rendered as chips — and only the first resembles confidence;
the third is epistemic *condition*, so the names say what they are:
- **Support**: single-source / corroborated / authoritatively-attested — derived
  from lineage (independent-lineage corroboration count) rather than
  agent-asserted wherever possible.
- **Coverage**: observed / partial / blind.
- **Validity**: fresh / stale / contested / superseded.
...
self-ancestry reinforcement (reachability check — needs lineage edges first);
...
semantic completeness permanently unknowable); three-way confidence split (columns
exist; values are LLM-authored); **no hidden adaptation** (spec §81, added in
...
1. Event schema v1: the four objects with observation_kind subtypes from day one
   (D7); four timestamps (ingestion = core-stamped seq + advisory wall clock with
   ...
   channels (review attestation vs firsthand assertion, D8); typed lineage edges
...
3. Deterministic migrator (zero LLM calls): concepts → Beliefs + attestation
   events per D8. Acceptance: **byte-identical re-projection of every demo-vault
...
1. write_concept becomes a proposal auto-applied at LOW risk through the extended
   mcp.rs validation chain — **the MCP tool surface is unchanged; the CLI agent
   notices nothing**. verify_concept emits attestation events.
2. IPC-boundary human-assertion capture for in-app edits (the app is the dominant
   editor; this path needs no heuristics) — before any watcher capture.
3. Projection manifest + launch scan+hash-diff reconciliation (with the circuit
   breaker) — then, and only then, live out-of-band watcher capture.
4. guard_human_write's successor: capture-valve semantics for projections.
Exit: a human edit of a projection becomes an assertion event; projection
...
3. expected_version on epistemic rows; conflict → structured rejection →
   re-proposal, never silent overwrite.
...
5. **Creation qualification** (spec §15, enforceable not aspirational): a
   create_belief proposal requires `candidate_matches_considered` +
   `distinctness_reason` — the path is exact-identity → semantic candidate →
   scoped/temporal candidate lookup → update/qualify existing if defensible →
...
1. Parse usage from every stream-json result event (currently discarded);
   per-run/per-lane accounting in the runtime DB; daily ambient budget with
   lane-priority degradation. Attended chat never budget-blocked; ambient always.
2. Scheduler state (processed-hash ledger, event cursor, pending set, backoff)
   moves from localStorage into the runtime DB — state loss means re-hashing, not
   re-spending. Launch catch-up = hash-diff through the budget gate (fixes the
   first-scan-yields-zero-events blindness for the app-closed period).
3. Content-hash + per-field materiality prefilter in Rust: git operations and
   ...
   is forbidden; the prefilter gates LLM spend, never epistemic recording.
...
1. Batched ingest pass replacing per-note distill jobs; proposals via
   ...
   bounded in-session retry on typed rejection. The pass includes the **basic
   ...
   *unresolved*, never guessed — without this, observations cannot attach to
   ...
   alias models stay M28+.
1b. **Source content is data, never instruction** (spec §92): ingest prompts
   quote source material as data; injection-shaped content (text addressed to
   an AI agent) is recorded as an observation property and logged, never
   executed. The structural backstop already exists — nothing an agent reads
   can mutate state except through M24's policy-validated proposals — but the
   prompt-level discipline and the logging are explicit deliverables here.
   Detection telemetry is recorded as `suspected_instructional_content` (+
   classifier id) — never `prompt_injection: true`; the boundary is the
   guarantee, detection is only telemetry.
...
3. Query-time assembly (attended-only), with the full contract (matrix Rev 2
   ...
   - **Contradiction-aware assembly** (context-integrity invariant): positive,
     ...
     — the Skeptic can wait; counterevidence retrieval cannot.
   ...
   - **Structured synthesis fields** (spec §98/§36): basis,
   ...
   - **Structured ephemeral discovery plans** (spec §68): goal/steps/stop_when/
     stakes as schema, not prose; persistence waits for Discovery objects.
...
5. **Budgets are multi-dimensional — run count alone is not a budget** (ten runs
   ...
   deterministic caps per pass — max_sources_per_run, max_context_bytes,
   max_evidence_items, max_daily_runs — so parsed CLI usage becomes accounting
   and alerting, never the only guardrail. Budget on runs + input context +
   output tokens + wall-clock/quota failures.
...
1. Support/Coverage/Validity ordinal chips (D9), with **default predicate-class
   freshness rules** so Validity means something (spec §45: charter rationale —
   durable · CI status — hours · shipping BOM — days/revision-bound); freshness
   semantics are not connector-dependent. Learned per-predicate policies M28+.
2. Lineage-independence corroboration (four copies of one Slack message = one
   ...
   analogue of availability ≠ completeness). Anti-self-reinforcement
   reachability check at proposal-apply.
3. **Scope resolution BEFORE contradiction.** A candidate contradiction must
   ...
   lag, not conflict. Without this step, contradiction preservation screams at
   ...
   resolved_by_granularity / genuine_direct / partial / conditional — several
...
Risk-gated Skeptic pass (after the meter shows what tiers cost; MEDIUM amortized
...
(≥2 live connectors) · always-on service · multi-master ledger merge (explicitly
...
- **Ledger vocabulary creep**: event schema changes are migrations of an
  append-only store — additive-only discipline from day one.
