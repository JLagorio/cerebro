...
## CORRECTION (M29.53): the verdicts are attached to the wrong findings

**The `Confirmed` and `Refuted` sections below are grouped correctly and
_titled_ wrongly.** This file was generated by zipping two lists — the probe
agents' findings and the verifiers' records — in different orders, so within
each entry the `Surface` / `Repro` / `Measured` / `Suspected cause` fields
belong to the heading above them, while `Re-measured by the verifier` and
`Verifier's reasoning` describe *a different finding entirely*.

It is unambiguous once you read the verifier text: it always names what it
measured. Here is what each verdict actually attaches to.

**Confirmed (9)** — the verifier tried to refute and could not:

| Verifier record filed under | Actually confirms |
|---|---|
| Escape does not leave the block's visual editor | the narrow-window block chrome overhang (**broken at 900px**, the app's own minimum: the Auto-layout control renders 147px past the card, under the outline panel, and three real clicks at its pixels do nothing) |
| The floating code panel swallows Escape | the uncommitted code draft thrown away on navigation (**broken**) |
| A popover stays put when the canvas is wheel-zoomed | Backspace deleting the ENTIRE mermaid block (**broken**, 842 → 700 bytes) |
| With Auto-update off, closing the dialog discards the draft | ⌘Z after a visual edit being a no-op — and it is WIDER than filed: a rename committed with Enter and the block's own Done both leave focus where ⌘Z is dead |
| Any block edit rewrites a prose line | the truncated error banner. Its other half — "816 characters dumped into the document" — is REFUTED: that card is deliberate, test-locked, never reaches disk, and is 124px SHORTER than the diagram it replaced |
| A link badge click destroys the dialog | Escape not leaving the block's visual editor (severity **rough**) |
| Escape with a node selected closes the editor | the clipboard carrying the block's chrome instead of the fence |
| The dialog toolbar clips "Save PNG…" | double-click on a diagram doing nothing |
| Closing the dialog drops focus into the doc body | ⌘K/⌘J dead in the block's source box AND in the .mmd code panel |

**Refuted (5)** — do not fix these:

| Verifier record filed under | Actually refutes |
|---|---|
| Backspace deletes the ENTIRE mermaid block | the Ask AI toolbar over the prose — not mermaid-specific, and 39% LESS obscuring than the text-selection toolbar that ships |
| One Escape closes Quick Open AND the dialog | diagrams being left-aligned — already fixed by `mx-auto` in M29.52 |
| A doc ending in a mermaid block has no trailing paragraph | double-clicking "Save as file…" writing two files — deliberate, documented, and guarded by a checked-in test |
| Resizing the window strands the diagram | the gantt being illegible inline — already fixed by `useLegibleWidth` in M29.52 |
| There is no route to full screen while editing | a doc ending in a mermaid block having no trailing paragraph — true of every doc in the vault, and two mouse-only routes work |

So five of the nine titles printed under `Confirmed`, and four of the five
printed under `Refuted`, carry NO verdict of their own — they are unverified
leads like the other 57. Of those, three were driven and fixed in M29.53
anyway (Quick Open's double-close, the toolbar clip, no-route-to-full-screen);
the popover-detach-on-zoom and focus-restore ones are still open.

The `Suspected cause` field is the reporter's own and stays with its heading —
it was never part of the misjoin.
