...
| **M34.3** | Handoffs — agent invokes agent — **DELIVERED 2026-08-20** (`e7ebce6` M34.3.1, `2d6f43e` M34.3.2; no dedicated plan doc — folded into the M34.0 recon) | The centerpiece of the settled design, shipped: a run-starting MCP tool; series only; two hops fixed in code, a cycle refuses and names the agent it refused; `parent_run_id` migration so hops bill to the root and the root's ceiling gates the chain; a paused agent cannot be called and the chain stops with it — enforced at the call, not the UI. M41.2 later SELECTed the hop lineage end to end for the Agents surface. |
| **M34.4** | Read scoping — **DELIVERED 2026-08-20** (`791ba4c`; no dedicated plan doc — folded into the M34.0 recon) | `get_note` / `search_notes` / `knowledge_about` / `list_inbox` honor the grant: withheld notes leave the corpus before ranking, withheld captures are counted not absorbed. M36 later added the AgentEditor read-scope row + prompt sentence surfacing this boundary in the UI. |
...
| **M41** | The Agents surface — **DELIVERED 2026-08-21** (plan: `plans/2026-08-21-m41-agents-surface.md`; requested after the M34–M40 delivery: config in Library, runs under Base, addressing in the panel — three rooms, no house) | The `agents` kind (actor = subject, so constructs get pages), the **Agents** destination after Inbox (the contract goes to TEN), AgentsPage composing the reused `AgentRoster` (a row is a destination here, not a filter) over the reused `FleetSection`, and one page per agent: charter, a grants summary from the same `agentDraft` the editor edits, pause/resume, and the run history with **chains rendered** — `parent_run_id` finally SELECTed end to end (fleet.rs + twin; M34.3 wrote it and nothing read it). Constructs say permanently-internal; a dangling actor is absent, said as absent. Creating/editing stay the Library's one flow — the surface is the door to it. |
...
| **M42** | The shell speaks the design system — **DELIVERED 2026-08-21** (plan: `plans/2026-08-21-m42-ds-shell.md`; driven by the user against `docs/cerebro-ds`: "it doesn't look like our design system at all… everything should be nested under its group like in Notion") | Paint and geometry, no engine: the sidebar is the DS's sunken surface with `--surface-selected` + cortex ink for the current row and quiet mono counts (the Inbox pill died — "numbers are quiet facts"); every destination that owns subjects is a GROUP whose rows nest under it on every surface — agents (+ New agent), mounted repos, prototypes, and Base's whole nav (which grew a `current` prop so an un-current nav lights nothing) — open by default, folded by a rotating chevron that never navigates, persisted as the CLOSED set; the roster and run feed lost their caps boxes, border boxes and raw selects for tiles, washes, hairlines, sentence-case chips and the DS `Select`; the agent page opens on a synapse tile header; and the ⌘K bar keeps the DS ask-bar promise — synapse sparkle, "Search or ask…", one pinned Ask row in the palette (`quick-open-ask`, an offer rather than a result, defaulting the highlight to the best match so Enter still opens the thing you named). |
