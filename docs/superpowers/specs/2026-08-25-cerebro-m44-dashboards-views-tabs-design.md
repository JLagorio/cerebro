...
**Date:** 2026-08-25
**Status:** approved for plan docs 2026-08-27 — gating product calls decided in §5
...
| --------- | ---- | --------- |
| **M44.1** | Customize display — per-type persisted detail-panel config. Smallest slice, no engine change, reuses the ViewSettingsPanel toggle-row pattern on a new surface. | Per-type display-config object (hidden-field keys, `showEmpty`/`showFile`/`showBody`, width preset); a settings sub-panel reachable from the detail panel's actions menu; wiring `RecordProperties`/`DetailPanel` to read/respect it; a reset-to-defaults action. Storage location: Type-doc frontmatter (decided, §5). |
...
## 5. Decisions (2026-08-27, design owner)

The five gating calls, decided before any M44.x plan doc:

- **Q10 + Q16 (storage, decided once for all three schemas):** Type-doc
  frontmatter, like `views:` — git-visible, portable, vault-first. Only
  device-local ephemera (e.g. `detailWidth`) stays in uiStore. M44.1's
  display config, M44.4's dashboard layout, and M44.5's tab sets all follow
  this one answer.
- **Q1 (chart X axis):** Yes, decouple. `ChartSpec.xField` + `ChartSpec.groupBy`
  as independent slots; `computeChart` stops deriving its axis from
  `presentation.group`. This deliberately reverses M16.27 — the constraint's
  comment in `src/engine/chart.ts` dies with the change (see AGENTS.md on
  retired workarounds). M44.2 still ships first and does not depend on it.
- **Q5 (dashboard model):** Rows/cells replaces flat `blocks[]`, with a
  migration for existing dashboards. `kind:'view'` embedding survives as a
  9th widget kind alongside the design's 8 own-scope kinds.
- **Q12 (tab scope):** Per TYPE — all records of a type share one tab set,
  matching the `views:`/Customize-display precedent and the design's own
  kind-family sets. A per-record override is a possible later layer, not M44.
- **Q6 (Portfolio dashboard / M44.6):** Out of M44. OKR/Key-Results is its
  own future milestone; nothing in M44 builds toward it.

The remaining open questions (2–4, 7–9, 11, 13–15) are plan-level: each
M44.x plan doc resolves the ones it touches, with a recommendation recorded
