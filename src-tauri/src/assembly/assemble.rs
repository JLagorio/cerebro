...
/// The assembly's identity: this question, for this use, under these caps,
/// against this base. Server-derived and content-addressed, so re-asking an
/// unchanged question against an unchanged ledger is idempotent.
...
mod tests {
    ...
    use crate::ledger::reduce::{
        AliasState, BeliefState, ObservationState, RelationState, RevisionState,
    };
    use crate::ledger::schema::{
        AssertionBasis, BasisLink, BasisRole, IntendedUseKind, ObservationKind, Qualification,
        Risk, Stage, SubjectRef,
    };
    ...
    const B_KESTREL: &str = "b0000000000000000000000000000003";
    const REV_ONE_OLD: &str = "10000000000000000000000000000001";
    const REV_ONE: &str = "10000000000000000000000000000002";
    const REV_TWO: &str = "10000000000000000000000000000003";
    const REV_KESTREL: &str = "10000000000000000000000000000004";
    const OBS_AUTHORITY: &str = "20000000000000000000000000000001";
    const OBS_INFERRED: &str = "20000000000000000000000000000002";
    const OBS_PLANNED: &str = "20000000000000000000000000000003";
    const SOURCE_A: &str = "50000000000000000000000000000001";
    const SOURCE_B: &str = "50000000000000000000000000000002";
    const SOURCE_C: &str = "50000000000000000000000000000003";
    ...
    fn revision(revision: u64, event_id: &str, content: &str, basis: BeliefBasis) -> RevisionState {
        RevisionState {
            revision,
    ...
    fn linked(observations: &[&str]) -> BeliefBasis {
        BeliefBasis::Linked {
            links: observations
                .iter()
                .map(|id| BasisLink {
    ...
    fn belief(id: &str, entity: &str, revisions: Vec<RevisionState>) -> BeliefState {
        BeliefState {
    ...
    fn assertion(event_id: &str, statement: &str, scope: Scope) -> super::super::corpus::Assertion {
        super::super::corpus::Assertion {
            event_id: event_id.to_string(),
    ...
    fn limits(sources: u64, bytes: u64, items: u64) -> Limits {
        Limits {
    ...
    fn request<'a>(scope: Scope, limits: Limits) -> Request<'a> {
        Request {
            store_uuid: STORE,
            chain_head: HEAD,
    ...
    fn assembled(request: &Request<'_>) -> Assembly {
