...
describe('addGroup', () => {
  ...
  it('mints a section with nowhere to put it — a group has no tab (M46.1)', () => {
    // Two arguments, and the minted group wears exactly three keys. A
    // section stands above the tab strip on every tab, so there is no home
    // for the caller to name and no key for the serializer to write.
    expect(addGroup).toHaveLength(2);
    const next = addGroup(base(), ['group-1', 'group-3']).layout;
