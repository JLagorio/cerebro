...
import type { CardPreview, CardSize, Presentation, ViewType } from '@/engine/types';
...
/**
 * Layouts that draw records as cards, and therefore have card settings.
 *
 * `satisfies Record<ViewType, boolean>` for the same reason `CAPABILITIES` in
 * viewKinds.ts uses it: a new view kind must ANSWER this, not inherit a
 * default and silently ship with no settings. It belongs on `ViewKind` beside
 * `chips` and `groupable`, and folding it in is a one-line move once the kind
 * table and this phase are not being edited at the same time.
 */
const CARD_LAYOUTS = {
  table: false,
  list: false,
  ...
  gantt: false,
  timeline: false,
} satisfies Record<ViewType, boolean>;

export function showsCards(type: ViewType): boolean {
  return CARD_LAYOUTS[type];
}
